...
# === DROPDOWN: Weapon selection ===
    ...
    def on_weapon_select(choice):
        ...
        try:
            print("trying to load_spray_pattern")
            pattern, steps, step_delay = pattern.load_spray_pattern(f"{choice}.txt")
    ...
    def toggle_debug_window():
        config.show_debug_window = debug_var.get()  # get show_debug_window 0 or 1

        # Check show_debug_window condition
        if config.show_debug_window == 1:
            print("activated")
        else:
            print("deactivated")
