...
def find_app_window(title_keyword="e2eSoft iVCam"):
...
cv2.resizeWindow("YOLOv8 Detection", 400, 400)

# Захват экрана по координатам окна приложения и обработка модели
def capture_and_predict(game_window, prev_time):
    # Получаем координаты окна приложения
    x, y, width, height = game_window.left, game_window.top, game_window.width, game_window.height

    # Используем mss для захвата экрана по координатам окна приложения
    with mss.mss() as sct:
...
# Поиск окна приложения и запуск детекции
game_window = find_app_window("e2eSoft iVCam")
...
if game_window:
    ...
    print("Не удалось найти окно. Убедитесь, что процесс запущен и окно доступно.")
